<template>
  <div class="container">
    <div class="search_group">
      ...
      <el-button @click="searchMovie" :icon="Search" />
    </div>
    <div v-if="data.list && data.list.length > 0" class="search_res">
      ...
      <div class="content">
        <div class="film_item" v-for="m in data.list" :key="m.id">
          <router-link :to="{ path: '/filmDetail', query: { link: m.id } }" :style="{ backgroundImage: `url('${m.picture}')` }"></router-link>
...
watch(route, () => {
  refreshPage(route.query.search, route.query.current);
});

const play = (id: string | number) => {
  router.push({ path: '/play', query: { id, episode: 0, source: 0 } });
};
...
const searchMovie = () => {
  ...
  router.push({ path: '/search', query: { search: data.search } });
};

const refreshPage = (keyword: any, current: any) => {
  ApiGet('/searchFilm', { keyword, current }).then((resp: any) => {
    if (resp.code === 0) {
...
const changeCurrent = (currentVal: number) => {
  router.push({ path: '/search', query: { search: route.query.search, current: currentVal } });
};
</script>
